...
-- Revenue Analysis
SELECT ROUND(SUM(unitprice), 2) AS TotalRevenue
FROM onlineretail;

SELECT ROUND(SUM(UnitPrice) / COUNT(DISTINCT InvoiceNo), 2) AS AvgOrderValue
FROM onlineretail;

SELECT Country, ROUND(SUM(UnitPrice)) AS Revenue
FROM onlineretail
GROUP BY Country
